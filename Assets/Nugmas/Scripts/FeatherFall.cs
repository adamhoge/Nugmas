...
public class FeatherFall : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        GetComponent<ConstantForce2D>().force = m_AntigravityForce * m_FloatForce;

        Vector3 center = m_Collider.offset;
    ...
    private void Puff()
    {
        ...
        if (downwardVelocity > 0.001f)
        {
            ...
            m_Rigidbody.AddForceAtPosition(m_LastPuffPower, puffPosition, ForceMode2D.Impulse);
        }
    }
    ...
    private Vector3 GetPuffPosition()
    {
        ...
        List<Vector3> worldEdges = m_EdgePoints
            .Select<Vector3, Vector3>(transform.TransformPoint)
            .ToList();
    ...
    {
        float totalMass = transform.GetComponentsInChildren<Rigidbody2D>().Sum(rb => rb.mass);
